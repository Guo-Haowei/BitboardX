...
const fn mirror_square(sq: u8) -> u8 {
    sq ^ 56
}

const fn init_mg_table(
    pst_table: &[[i32; 64]; 6],
    piece_value_table: &[i32; 6],
) -> [[i32; 64]; 12] {
    let mut table = [[0; 64]; 12];
    let mut wp = 0;
    while wp < 6 {
        let mut sq = 0;
        let piece_value = piece_value_table[wp];
        while sq < 64 {
            let mirror = mirror_square(sq);
            let bp = wp + 6;
            table[wp][sq as usize] = pst_table[wp][sq as usize] + piece_value;
            table[bp][sq as usize] = pst_table[wp][mirror as usize] + piece_value;
...
pub const MG_TABLE: [[i32; 64]; 12] = init_mg_table(&MG_PIECE_TABLES, &MG_PIECE_VALUE);
pub const EG_TABLE: [[i32; 64]; 12] = init_mg_table(&EG_PIECE_TABLES, &EG_PIECE_VALUE);
...
mod test {
    ...
    fn test_mirror_square() {
        assert_eq!(mirror_square(Square::A1.as_u8()), Square::A8.as_u8());
